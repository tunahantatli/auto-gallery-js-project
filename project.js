...
function addCar(e) {
  ...
  if (title === "" || price === "" || url === "") {
    ui.displayMessage("Fill in all empty fields!", "warning");
  } else {
    //new car
    const newCar = new Car(title, price, url);

    ui.addCarToUI(newCar); // this operation: add new car to interface
    ui.displayMessage("Car successfully added!","success")
  }
...
UI.prototype.displayMessage = function (message, type) {
  const cardBody =
    document.querySelector(".card-body"); /* we have 2 card-body class 
  ...
  cardBody.appendChild(div); /* we add div but we want it to disapper after a certain time. 
